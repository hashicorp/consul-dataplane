...
#
# Verifies that the released consul-dataplane Docker image is available
# in the HashiCorp registry and reports its version and image digest (SHA).
#
# Workflow:
#   1. Pull  hashicorp/consul-dataplane:<version>  (prints pull output)
#   2. Print the image digest (sha256) and image ID
#   3. Run   `consul-dataplane -version` inside the image (prints output)
#
# Usage:
#   ./verify-release-image.sh [staging|production]
#   DP_PRODUCT_VERSION=2.0.2 ./verify-release-image.sh staging
#
...
# Setting DP_PRODUCT_VERSION in the environment pre-fills the default.
...
# -----------------------------------------------------------------------------
DEFAULT_DP_PRODUCT_VERSION=2.0.2
...
if [[ "${ENVIRONMENT}" == "staging" ]]; then
  IMAGE_REPO="${IMAGE_REPO:-crt-core-staging-docker-local.artifactory.hashicorp.engineering/docker.io/hashicorp/consul-dataplane}"
else
  IMAGE_REPO="${IMAGE_REPO:-hashicorp/consul-dataplane}"
fi
...
prompt_var DP_PRODUCT_VERSION \
  "${DP_PRODUCT_VERSION:-${DEFAULT_DP_PRODUCT_VERSION}}"
echo

IMAGE_REF="${IMAGE_REPO}:${DP_PRODUCT_VERSION}"
...
# -----------------------------------------------------------------------------
# 3. Print the consul-dataplane version from inside the image
# -----------------------------------------------------------------------------
