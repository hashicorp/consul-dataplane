...
echo "git merge-base origin/$skip_check_branch origin/main: $(git merge-base origin/$skip_check_branch origin/main)"
echo "git diff --name-only $(git merge-base origin/$skip_check_branch origin/main)...HEAD: $(git diff --name-only "$(git merge-base origin/$skip_check_branch origin/main)"...HEAD)"

files_to_check=$(git diff --name-only "$(git merge-base origin/$skip_check_branch origin/main)"...HEAD)
